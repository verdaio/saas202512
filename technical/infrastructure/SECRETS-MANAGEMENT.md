...
**Navigate to:**
```
https://github.com/YOUR_ORG/saas202512/settings/secrets/actions
```
...
| `AZURE_REGISTRY_NAME` | Container Registry name | `saas202512acr` |
...
az containerapp restart --name saas202512-staging --resource-group saas202512-rg
az containerapp restart --name saas202512-prod --resource-group saas202512-rg
```
...
az ad sp create-for-rbac \
  --name "saas202512-github-actions-new" \
  --role contributor \
  --scopes /subscriptions/{sub-id}/resourceGroups/saas202512-rg \
...
1. Verify secret exists:
   ```bash
   az keyvault secret show --vault-name saas202512-kv --name SECRET-NAME
   ```
...
2. Check access policy:
   ```bash
   az keyvault show --name saas202512-kv --query properties.accessPolicies
   ```
...
az keyvault secret set --vault-name saas202512-kv --name SECRET-NAME --value "value"

# Get secret from Key Vault
az keyvault secret show --vault-name saas202512-kv --name SECRET-NAME --query value -o tsv

# List all secrets
az keyvault secret list --vault-name saas202512-kv

# Delete secret
az keyvault secret delete --vault-name saas202512-kv --name SECRET-NAME
